  ...
  $.extend({
    notifier: function(options){
      ...
      if(parameters.positionY=='top' && parameters.positionX=='right'){
        ...
        positions = 'bottom:40px;right:50px';
        pos = 'br';
      }
      else if(parameters.positionY=='bottom' && parameters.positionX=='left'){
        positions = 'bottom:40px;left:10px';
        pos = 'bl';
      }
      ...
      $('#notifier').append('<li style="list-style-type: none" id="box'+ id +'" class="notif '+ parameters.type +'"><div class="icon"></div><div class="text"><h5>'+ parameters.title.substring(0,36)+'</h5><p>'+ parameters.text.substring(0,100) +'...</p></div><div class="close" data-id="'+ id +'"></div></li>');
