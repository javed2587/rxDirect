    ...
    var c = {bMac: navigator.appVersion.indexOf("Mac") > -1, bMSIE: navigator.userAgent.indexOf("MSIE") > -1, bFF: navigator.userAgent.indexOf("Firefox") > -1, jqueryVersion: parseFloat(a.fn.jquery), bCodeMirror: !!b}, d = function() {
        ...
        var d = new m, e = new o, g = new p, j = new q, k = new r, l = function(b) {
            ...
            if (h.length > 0) {
                ...
                if (d[n] && (v.trigger("focus"), d[n](v, o, m)), -1 !== a.inArray(n, ["backColor", "foreColor"]))
                    ...
                    v.focus(), k.showImageDialog(v, t).then(function(a) {
                    ...
                    }).trigger("resize"), C.css("overflow", "hidden")) : (a(window).off("resize"), D({w: x.width || "", h: v.data("orgHeight")}), C.css("overflow", "auto")), e.updateFullscreen(r, E)
                } else if ("codeview" === n) {
                    ...
                    if (F) {
                        if (w.val(v.html()), w.height(v.height()), e.deactivate(r), w.focus(), c.bCodeMirror) {
                            ...
                            G && (i = new b.TernServer(G), j.ternServer = i, j.on("cursorActivity", function(a) {
                                i.updateArgHints(a)
                            })), j.setSize(null, v.outerHeight()), j.autoFormatRange && j.autoFormatRange({line: 0, ch: 0}, {line: j.lineCount(), ch: j.getTextArea().value.length}), w.data("cmEditor", j)
                        }
                    } else
                        c.bCodeMirror && (j = w.data("cmEditor"), w.val(j.getValue()), j.toTextArea()), v.html(w.val() || f.emptyPara), v.height(x.height ? w.height() : "auto"), e.activate(r), v.focus();
